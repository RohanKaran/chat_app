...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        await self.update_user_status(self.user, self.user.current_connection, is_online=True)
    ...
    async def receive(self, text_data):
        ...
        await self.channel_layer.group_send(
            self.room_name, {"type": "chat_message", "message": message, "user": self.user.id}
        )
...
class FindConsumer(AsyncWebsocketConsumer):
    ...
    def find_match(self, user):
        ...
        for potential_match in potential_matches:
            if potential_match.id in FindConsumer.connected_users and not potential_match.current_connection:
                return potential_match

        any_online_user = (
            User.objects.filter(is_online=True)
            .exclude(id=user.id)
            .filter(id__in=FindConsumer.connected_users.keys())
            .first()
        )
